...
const linkComponent = (text?: string, path?: string, type?: TypeEnum) => {
    return (
        <div className={$styles.link}>
            <Link href={`${type === TypeEnum.EMAIL ? `mailto:${path}` : `${path}`} `} target={type === TypeEnum.LINK ? '_blank' : ''} className={$styles.link_label}>
                { text }
            </Link>
        </div>
    )
        ...
        list && list.length ? <>
            {
                list.map((items, index) => (
                    <div className={`${$styles.item_box} ${items.length == 1 ? $styles.large_box : ''}`} key={index}>
                        {
                            items.map((item, _i) => (
                                <div className={$styles.item_box_item} key={item.iconPath}>
                                    <div className={$styles.item_box_item_inner}>
                                        ...
                                        <div className={$styles.item_box_item_inner_reverse}>
                                            { item.reverseImg ? <Reverse reverseImg={item.reverseImg}/> : <></>}
                                            { !isNil(item.path) ? linkComponent(item.text, item.path, item.type) : <></> }
                                        </div>
                                    </div>
                                </div>
                            ))
