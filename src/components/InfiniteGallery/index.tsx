...
import { IMG_HEIGHT, IMG_WIDTH, IMG_MARGIN, COLS, ROWS, BORDER_RADIUS, FRAME_TIME, MAX_DIFF, MIN_DIFF } from "./constant"
import { IProps, IImageData, cell } from "./type"
...
const InfiniteGallery: FC<IProps> = (props: IProps) => {
    ...
    const [grid] = useState<cell[][]>(
        Array(rows).fill(null).map(
            (_, row) => Array(cols).fill(null).map(
                (_, col) => ({ row, col })
            )
        )
    )
    ...
    // 绘制圆角图片
    const drawRoundedImage = (ctx: CanvasRenderingContext2D, img: HTMLImageElement, x: number, y: number, imgWidth: number, imgHeight: number) => {
        if (!ctx) return;
        ctx.save();
        ctx.beginPath();
    ...
    const handleCanvasMouseDown = useCallback(() => { setMoveable(true); }, []);

    const handleCanvasMouseUp = useCallback(() => { setMoveable(false); }, []);

    const handleCanvasMouseLeave = useCallback(() => { setMoveable(false); }, []);

    const handleCanvasMouseMove = useCallback(throttle((e: React.MouseEvent<HTMLCanvasElement>) => {
        if (!moveable) {
            return;
        }
    ...
    const handleTouchMove = useCallback(throttle((e: React.TouchEvent) => {
